...
<?php
  ...
  if(isset($_SESSION["commentJson"])){
    $json = $_SESSION["commentJson"];
    $isReply = true;
    unset($_SESSION['commentJson']);
  }
...
<!-- blog -->
	...
	<div class="container">
	  ...
	  <div id="online" class="online">
	    <div class="comments">
			<p>&nbsp;</p>
			<a name=“online”><h1>在线咨询窗口 YAO'S ONLINE</h1> 
            </a>
			<form id="form" action = <?php echo "'".LIB_PATH."/Home/index/postComment' "?> method="POST" onsubmit="return check(this)">
				<p>
          <input type="hidden" id="toCommentField" name="toComment" value="" />
				  <input style="margin: 10px 0" type="text" placeholder="Name" name="username" id="username" required>
				  <input style="margin: 10px 0" type="text" placeholder="Email" name="mail" id="mail" required>
          ...
          if($isReply){
                $array = array($json);
          }else if($client -> get(LIB_PATH."/Home/index/getGlobalComment/number/4")){
          ...
          ?>
                <div class="responses-response-fig">
                  <div class="response-fig"> <span> </span> </div>
                  <div class="response-fig-text">
                    ...
                    <p class="paragh"><?php echo $item["content"] ?></p>
                    <?php if($isAdmin){?>
                    <a href="#form" onclick='changeFormValue(<?php echo $item["id"]?>)' onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image12','','images/7D02.png',1)">回 复</a>
                    <?php }?>
                    </div>
                  <div class="clearfix"> </div>
                </div>
               <?php } ?>
                <h2><a name="“mail”">在线咨询渠道</a></h2>
                <div class="footer-middle">
                  <ul>
                    <li>
                      <h2><a href="#" class="twit"> </a></h2>
                    </li>
                  </ul>
                </div>
                <h2>&nbsp;</h2>
